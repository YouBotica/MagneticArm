...
class PositionContollerClient(Node):

    def __init__(self):
        super().__init__('position_controller_client')
    ...
    def send_goal(self, angle):
        ...
        point2.time_from_start = Duration(seconds=8, nanoseconds=0).to_msg()
        point2.positions = [1.2, 0.8]

        points.append(point1)
        points.append(point2)

        goal_msg.goal_time_tolerance = Duration(seconds=10, nanoseconds=0).to_msg()
    ...
    def feedback_callback(self, feedback_msg):
        ...
        self.actual_angles = [float(feedback.actual.positions[0]), float(feedback.actual.positions[1])] #, float(feedback.actual.positions[2]), float(feedback.actual.positions[3]), float(feedback.actual.positions[4]), float(feedback.actual.positions[5])] 
...
def main(args=None):
    ...
    action_client = PositionContollerClient()
